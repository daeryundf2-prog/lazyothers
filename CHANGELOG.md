...
- `scripts/verify_legal_factuality.py`:
  ...
  * **Section 5.1 #4 불가능한 사법 절차 날조 차단**: 대한민국 실정법(형사소송법 제448조, 제246조, 헌법 제12조, 제111조 등)상 성립할 수 없는 사법 절차(`대검찰청의 약식명령 청구`, `고등검찰청의 약식기소`, `경찰의 영장 직접 청구`, `경찰의 직접 기소/공소제기`, `헌법재판소의 징역형 선고`, `민사소송에서의 징역형 선고`, `형사소송의 원고` 등)를 문장 내 장문 복문 수식어(25자 이상)와 상관없이 탐지하며, 상급 검찰청의 합법적인 수사 지휘/검토 지시 문맥은 허용하도록 음성 룩어헤드 완비.
  * **Section 7-8 종합 헬스체크 CLI (`--health-check`)**: 법령 조문, 판례 연도, 법원 명칭, 관공서 및 학술지 명칭, 역사적 사건(한자 차수 포함), 사법 절차(장문 복문 포함), Evidence-First 프로토콜 전수 10대 카테고리 기계적 테스트 및 100점 만점 판정 시스템 신설.
- `skills/legal-draft-builder/SKILL.md`: Section 5.1 #3 & #4 방어 프로토콜 및 `--health-check` 명령어 가이드, 트리거 동기화 추가.
- `tests/test_verify_legal_factuality.py`: 역사 사건 날조 차단, 한자 차수/단일 사건 차단, 사법 절차 날조 차단(장문 복문), 학술 인용 날조 차단, 종합 헬스체크 100점 검증 테스트 추가 (총 32개 전수 통과).
